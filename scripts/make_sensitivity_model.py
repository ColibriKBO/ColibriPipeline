...
if __name__ == '__main__':
    ...
    #data_path = base_path.joinpath('tmp', 'AirmassSensitivity', str(obs_date).replace('-', ''))    #path to data
    data_path = base_path.joinpath('tmp', 'airmassssensitivity') 

    
    # minute_dirs=[f.name for f in data_path.iterdir() if ('dark' not in f.name and '.txt' not in f.name)]
    minute_dirs = ['20240830_03.05.33.668']
    minute_dirs_datetime = [datetime.strptime(f, MINUTEDIR_STRP) for f in minute_dirs]

    # testing
    # minute_dirs = minute_dirs[:5]
    # 20240729_03.11.46.296
    ...
    for minute_dir in minute_dirs:
        ...
        save_path = base_path.joinpath(data_path, '25ms', str(obs_date).replace('-', '') + '_diagnostics', 'Sensitivity', minute_dir)       #path to save outputs in
        ...
        folder = data_path.joinpath(minute_dir)
        print(folder)
        filename = sorted(folder.glob("*.rcd"))[0]
        filename_str = filename.name
        time_part = filename_str.split('_')[1]
        ...
        if transform_file.exists():
            ...
            try:
                ...
                print("Local solution failed, trying online")
        ...
        final.to_csv(save_path.joinpath('starTable_' + minute_dir + '_' + gain + '_' + polynom_order + '_' + telescope + '_' + str(detect_thresh) +'_'+str(airmass)+'_' + str(time_part)+'_' +'sig.txt'), sep = ' ', na_rep = 'nan')
        ...
        # except:
        #     print(f"Minute dir {minute_dir} failed. Moving to next day")
        #     with open('log.txt', 'a') as log_file:
        #         log_file.write(f"Minute dir {minute_dir} failed.\n")
