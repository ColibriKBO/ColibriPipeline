...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('-p', '--path_to_file', help= 'Path to a file containing all observed data', default='/ColibriData') # This addition makes allows the code to be used for more gerenal porpuses. Made by Toni C. Almeida.
    ...
    # By default = file_path = '/ColibriData'. But if it is needed it is possible to change its value.
    file_path = cml_args.path_to_file # Toni C. Almeida
    base_path = pathlib.Path(cml_args.basedir)
    data_path = base_path.joinpath(file_path, str(obs_date).replace('-', ''))    #path to data
    ...
    save_path = base_path.joinpath(file_path, str(obs_date).replace('-', '') + '_diagnostics', 'Sensitivity', minute_dir)       #path to save outputs in
    ...
    #input('Press enter if you have performed the astrometric solution')

    transform_file = save_path.joinpath('wcs.fits') #path to save WCS header file in
    transform_str=str(transform_file).split('\\')[-1]
